...
public class SaleByMonthAdapter extends RecyclerView.Adapter<SaleByMonthAdapter.SaleByMonthViewHolder>{
    ...
    public SaleByMonthAdapter(List<MasterSaleModel> list, Context context) {
        this.list = list;
        this.context = context;
    }

    @Override
    public SaleByMonthViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //Inflate the layout, initialize the View Holder
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_rp_sale_by_date_month, parent, false);
        SaleByMonthViewHolder holder = new SaleByMonthViewHolder(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(SaleByMonthViewHolder holder, int position) {
        //Use the provided View Holder on the onCreateViewHolder method to populate the current row on the RecyclerView
        holder.tvDate.setText(list.get(position).getMonthName());
    ...
    public class SaleByMonthViewHolder extends RecyclerView.ViewHolder {

        public TextView tvDate,tvTotalSale,tvTotalQuantity,tvTotalAmt,tvDiscount,tvNetAmt;

        public SaleByMonthViewHolder(View itemView) {
            super(itemView);
