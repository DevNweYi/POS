...
import adapter.PayableListAdapter;
import common.SystemInfo;
import database.DatabaseAccess;
import model.SupplierModel;

public class PayableListActivity extends AppCompatActivity {
    ...
    PayableListAdapter payableListAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_payable_list);
        db=new DatabaseAccess(context);
        ActionBar actionBar=getSupportActionBar();
        actionBar.setDisplayHomeAsUpEnabled(true);
        setLayoutResource();
        setTitle(R.string.sub_payable_list);

        getSupplierDebtAmount(etSearch.getText().toString());
        tvTotalPay.setText(String.valueOf(systemInfo.df.format(db.getSupplierTotalDebtAmount())));

        lvPayable.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                supplierId=lstSupplier.get(i).getSupplierId();
                showPayDebtDialog(lstSupplier.get(i).getSupplierName(),lstSupplier.get(i).getDebtAmount());
            }
        });
    ...
    private void showPayDebtDialog(final String name, final int debtAmount){
        ...
        input_debt_amount.setHint(context.getResources().getString(R.string.payable));

        tvName.setText("To "+name);
        ...
        btnOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                if(paidAmount>debtAmount){
                    input_paid_amount.setError(getResources().getString(R.string.paid_greater_payable));
                    return;
                }
                int curDebtAmount = debtAmount - paidAmount;
                db.updateDebtAmountBySupplier(supplierId, curDebtAmount);
                db.insertPayable(supplierId,debtAmount,paidAmount,etDate.getText().toString(),etRemark.getText().toString());
                updateAdapter("");
                tvTotalPay.setText(String.valueOf(systemInfo.df.format(db.getSupplierTotalDebtAmount())));
    ...
    private DatePickerDialog showDatePicker(){
        ...
        DatePickerDialog datePicker=new DatePickerDialog(PayableListActivity.this,new DatePickerDialog.OnDateSetListener() {
            @Override
    ...
    private void getSupplierDebtAmount(String supplierName){
        lstSupplier = db.getSupplierDebtAmount(supplierName);
        payableListAdapter = new PayableListAdapter(this, lstSupplier);
        lvPayable.setAdapter(payableListAdapter);
    }

    private void updateAdapter(String supplierName){
        lstSupplier = db.getSupplierDebtAmount(supplierName);
        payableListAdapter.updateResults(lstSupplier);
    }

    private void setLayoutResource(){
        lvPayable=findViewById(R.id.lvPayable);
